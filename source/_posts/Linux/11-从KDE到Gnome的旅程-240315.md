...
# 安装`Gnome`
``` shell
pacman -S gdm xorg-xwayland gnome gnome-tweaks gnome-nettool gnome-usage gnome-multi-writer adwaita-icon-theme xdg-user-dirs-gtk #nautilus-sendto arc-gtk-theme
# 设置开机自启
...
```
当然上面的方案已经不建议用了，现在建议换成`pipewire`。~~*因为`pulseaudio`没`pipewire`好用*~~
``` shell
sudo pacman -Rs pulseaudio-bluetooth
sudo rm -r ~/.config/pulse ~/.local/state/pipewire ~/.local/state/wireplumber
...
> 注意：gnome自带的那些插件要安装`gnome-shell-extensions`才有。
...
# KDE使用遇到的问题
在nvidia机器上sddm可能会在开机的时候卡住，那我们需要让驱动在用户空间服务前就加载。
编辑`/etc/mkinitcpio.conf`，往`MODULES`里添加`nvidia nvidia_modeset nvidia_uvm nvidia_drm`。我的设备上是这样的：
``` conf
...
# 其他的小问题
## 不小心把`/etc`整个包括子目录全chmod 755了怎么办
检查权限有问题的文件：
``` shell
sudo pacman -Qkk | grep -E "warning:|permissions"
```
修复来自官方源的软件：
``` shell
...
修复来自forign的软件：
``` shell
paru -S --rebuild --overwrite='*' $(pacman -Qmq)
```



## 办公类软件字体缺失，补全后系统字体变得很奇怪
从Windows分区里把`C:\Windows\Fonts`文件夹复制到`~/.local/share/fonts`里，然后创建`.config/fontconfig/fonts.conf`并写入：
``` xml
<?xml version="1.0"?>
<!DOCTYPE fontconfig SYSTEM "urn:fontconfig:fonts.dtd">
<fontconfig>

  <!-- 强制 monospace 指向你想要的等宽（优先 Sarasa，再 Noto Mono 等） -->
  <alias>
  ...
  <!-- 明确告诉 fontconfig：当查找 CJK 字形时，先用 Noto Sans CJK，避免直接回退到 YaHei -->
  <match target="pattern">
    <test name="lang" compare="contains">
      <string>zh-cn</string>
    </test>
    <edit name="family" mode="prepend_first">
      <string>Sarasa Mono CJK SC</string>
    </edit>
  </match>

</fontconfig>
```
然后创建`vim ~/.config/fontconfig/conf.d/10-monospace-western-first.conf`并写入：
> 下面这个文件主要是修复`Alacritty`中字体错误用的，如果你遇到了别的软件字体也出现了错误，也可以尝试使用修复。
``` xml
<?xml version="1.0"?>
<!DOCTYPE fontconfig SYSTEM "urn:fontconfig:fonts.dtd">
<fontconfig>
...
最后执行`fc-cache -fv`，系统字体和办公软件字体就都正常了。
...
## 随机化mac地址
创建并编辑`/etc/NetworkManager/conf.d/00-mymodify.conf`：
``` conf    
[device]
...
[connection]
# 随机mac地址
#wifi.cloned-mac-address=aa:12:34:aa:12:34 # 固定一个mac地址
wifi.cloned-mac-address=random # 管你什么网络，每次都随机
#wifi.cloned-mac-address=stable # 默认不同网络不同mac，但保持相同网络mac不变
# 自定义设备名
ipv4.dhcp-hostname=CXK
ipv6.dhcp-hostname=CXK
