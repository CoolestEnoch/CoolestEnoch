...
## 开启允许永久删除功能
``` shell
docker exec app bash -lc 'cd /var/www/discourse && rails runner '\''SiteSetting.can_permanently_delete = true; puts "can_permanently_delete=#{SiteSetting.can_permanently_delete}"'\'''
```


## 已删除的图片和资源文件
``` shell
docker exec -it app bash -lc 'cd /var/www/discourse && rake uploads:clean_up'
```
